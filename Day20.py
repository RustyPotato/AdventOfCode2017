inData = ["p=<-1724,-1700,5620>, v=<44,-10,-107>, a=<2,6,-9>",\
...
for index in range(len(inData)):
    pieces = inData[index].split(", ")
    posArray = [int(x) for x in pieces[0][3:-1].split(",")]
    velArray = [int(x) for x in pieces[1][3:-1].split(",")]
    accArray = [int(x) for x in pieces[2][3:-1].split(",")]
    
    inData[index] = [posArray, velArray, accArray]

print(inData[0:5])

minParticle = -1
slowestAccelleration = 999999999
for index in range(len(inData)):
    if ((abs(inData[index][2][0]) + abs(inData[index][2][1]) + abs(inData[index][2][2])) < slowestAccelleration):
        minParticle = index
        slowestAccelleration= (abs(inData[index][2][0]) + abs(inData[index][2][1]) + abs(inData[index][2][2]))
